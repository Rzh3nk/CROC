...
public final class ChessPositionParser {
    ...
    public static ChessPosition parse(final String position) throws IllegalPositionException {
        Chess chess = new Chess(position); //создали новый объект
        if (position.length()>2 || chess.x()<0 || chess.x()>7 ||chess.y()<0 || chess.y()>7){
            throw new IllegalPositionException();//проверяем возможен ли ход и кидаем  икслючение
        }
        return chess;
    }
}
